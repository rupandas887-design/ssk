...
const ManageOrganisations: React.FC = () => {
  ...
  const [newOrg, setNewOrg] = useState({ name: '', mobile: '', secretaryName: '', email: '', password: '', profilePhoto: null as File | null });
  ...
  const fetchOrganisations = useCallback(async () => {
    ...
    try {
      ...
      const { data: profiles } = await supabase.from('profiles').select('id, email, organisation_id').eq('role', 'Organisation');
  ...
  const handleAddOrganisation = async () => {
    ...
    try {
        // 1. Check for cross-role conflict (Volunteer Registry)
        const { data: profileCheck } = await supabase.from('profiles').select('id, role').eq('mobile', mobile.trim()).maybeSingle();
        if (profileCheck) {
          throw new Error(`The identity with mobile "${mobile}" is already registered as a ${profileCheck.role}. An entity can only be registered once in the system.`);
        }
        ...
        if (newOrg.profilePhoto) {
            ...
            const { data } = await supabase.storage.from('member-images').upload(fileName, newOrg.profilePhoto);
        ...
        const { data: orgData, error: orgError } = await supabase.from('organisations').insert({ 
            name: name.trim(), mobile: mobile.trim(), secretary_name: secretaryName.trim(), status: 'Active', profile_photo_url: photoUrl || undefined
        }).select().single();
        ...
        if (authData.user) {
            await supabase.from('profiles').upsert({
                id: authData.user.id, name: secretaryName.trim(), email: email.trim().toLowerCase(), role: 'Organisation',
                organisation_id: orgData.id, mobile: mobile.trim(), status: 'Active'
            });
        }

        await syncToSheets(SheetType.ORGANISATIONS, { name, secretary_name: secretaryName, mobile, email, status: 'Active' });
          ...
          <Card title="Authorize New Organization" className="border-orange-500/10 relative overflow-hidden">
            <div className="space-y-6 relative z-10">
              ...
              <div className="space-y-6">
                ...
                <Button type="button" onClick={handleAddOrganisation} disabled={isSubmitting} className="w-full py-5 flex items-center justify-center gap-3 text-[11px] font-black uppercase tracking-[0.3em]">
                ...
                <table className="w-full text-left">
                    ...
                    <tbody>
                        ...
                        <tr key={org.id} className="group border-b border-gray-900/50 hover:bg-white/[0.02]">
                            <td className="p-6">
                                <div className="flex items-center gap-4">
                                    <div className="h-12 w-12 rounded-xl overflow-hidden border border-white/10 bg-black/40 flex items-center justify-center">
                                      {org.profile_photo_url ? <img src={org.profile_photo_url} className="h-full w-full object-cover" /> : <Building2 size={24} className="text-orange-500/50" />}
                                    </div>
                            ...
                            <td className="p-6 text-right">
                                <button onClick={() => { setEditingOrg({ ...org }); setIsModalOpen(true); }} className="p-2.5 bg-white/5 rounded-xl border border-white/10 hover:border-orange-500/50 text-white">
                                    <Edit size={18} />
                                </button>
                            </td>
                        </tr>
      ...
      {showSuccessSplash && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none bg-black/40 backdrop-blur-sm animate-fade-in">
            <div className="relative animate-splash bg-[#0a0a0a] border border-orange-500/30 rounded-[3rem] p-12 flex flex-col items-center gap-6 shadow-2xl">
                <div className="h-24 w-24 bg-orange-500/10 rounded-full flex items-center justify-center text-orange-500 border border-orange-500/20"><PartyPopper size={48} /></div>
          ...
          {editingOrg && (
            <div className="space-y-6">
                ...
                <Button onClick={async () => {
                    setIsSubmitting(true);
                    const { error } = await supabase.from('organisations').update({ name: editingOrg.name, status: editingOrg.status }).eq('id', editingOrg.id);
                    if (!error) { fetchOrganisations(); setIsModalOpen(false); addNotification('Registry sync complete.', 'success'); }
                    setIsSubmitting(false);
                }} disabled={isSubmitting} className="w-full">Apply Changes</Button>
            </div>
          )}
      </Modal>
    </DashboardLayout>
